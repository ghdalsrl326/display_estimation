...
for s = 1:1 % 1번 피험자 ~ 10번 피험자 데이터
    for n = 1:1 % 1번 부모니터 위치 ~ 5번 부모니터 레퍼런스 위치 (우축, 중앙, 좌측, 좌측, 우측)
        ...
        fprintf(trial_fileID,"trials,Distance_Error(mm),Direction_Error(deg),ref_display_position_x,ref_display_position_y,ref_display_position_z,second_display_position_x,second_display_position_y,second_display_position_z,headgaze_coef\n");
        ...
        for t = 40:2:40 % 40 trial로 고정하려면 40:2:40 으로 대체, 각 모니터 최소 2회 필요하므로 최소값은 4, 최대값은 100
           ...
           %% 초기 5번 calibration 클릭 제외한 데이터 추리기
            ...
            results = bayesopt(fun,vars,'XConstraintFcn',@(x)xconstraint(x,sphere_center, A,b),'UseParallel',true, 'NumSeedPoints',128, 'MaxObjectiveEvaluations',50, 'AcquisitionFunctionName','expected-improvement-per-second-plus', 'ExplorationRatio',0.5);
            toc
            
            max_second_display_position = table2array(results.XAtMinObjective(:,1:3));
            max_headgazecoef = table2array(results.XAtMinObjective(:,4));
            max_sigma = table2array(results.XAtMinObjective(:,5));
            %% 결과 저장
