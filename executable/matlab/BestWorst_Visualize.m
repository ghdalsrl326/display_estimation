...
ref = readtable('../../dataset/user_test_case3_bo/ref_coord');
% ref = 레퍼런스 부모니터 중심점 위치가 담긴 파일의 경로
path = "user_test_case3_bo\";
...
%% 분석 시작
for s = 9:9 % 1번 피험자 ~ 10번 피험자 데이터
    for n = 1:1 % 1번 부모니터 위치 ~ 5번 부모니터 레퍼런스 위치 (우축, 중앙, 좌측, 좌측, 우측)
        ...
        for t = 100:2:100 % 40 trial로 고정하려면 40:2:40 으로 대체, 각 모니터 최소 2회 필요하므로 최소값은 4, 최대값은 100
       ...
       %% 추정 결과 시각화 (여기서부터는 grid_search_mle.m 파일과 동일)
        second_display_position = [-738,-228,154]; % 임의 부모니터 위치 입력
        coef = 4.105583; % head_gaze 파라미터 설정
...
%             plot3([head_origin_second(i,1), head_direction_second(i,1)], [head_origin_second(i,2), head_direction_second(i,2)], [head_origin_second(i,3), head_direction_second(i,3)],'Color', [0 0.4470 0.7410 0.25], 'LineWidth', 2);
%             plot3([head_origin_second(i,1) head_direction_second_calib(i,1)], [head_origin_second(i,2) head_direction_second_calib(i,2)], [head_origin_second(i,3) head_direction_second_calib(i,3)],'Color',[0.9290 0.6940 0.1250 0.25], 'LineWidth', 2);
            plot3([head_origin_second(i,1), head_direction_second(i,1)], [head_origin_second(i,2), head_direction_second(i,2)], [head_origin_second(i,3), head_direction_second(i,3)],'Color', [0 0.4470 0.7410 0.25], 'LineWidth', 3);
            plot3([head_origin_second(i,1) head_direction_second_calib(i,1)], [head_origin_second(i,2) head_direction_second_calib(i,2)], [head_origin_second(i,3) head_direction_second_calib(i,3)],'Color',[0.8500 0.3250 0.0980 0.4], 'LineWidth', 3);        
%             plot3([head_origin_second(i,1), 0], [head_origin_second(i,2), 0], [head_origin_second(i,3), 20],'Color', [0 0 0 0.25], 'LineWidth', 3);            
        end
                
        % 머리 방향벡터 - 주/부모니터 교점
        scatter3(head_second_intersection(:,1),head_second_intersection(:,2),head_second_intersection(:,3),'filled','c');
        scatter3(head_second_intersection_calib(:,1),head_second_intersection_calib(:,2),head_second_intersection_calib(:,3),'filled','r');
%         scatter3(d1_target_x,d1_target_y,zeros(length(d1_target_x),1),120,'filled','k','pentagram');
        scatter3(d2_target(:,1),d2_target(:,2),d2_target(:,3),'filled','k','^');
...
%         plot3([ref_display_position(1), second_display_position(1)], [ref_display_position(2), second_display_position(2)], [ref_display_position(3), second_display_position(3)],'k:','LineWidth',3);
        ...
        % 추정 부모니터 프레임
        plot3([est_c1(1) est_c2(1) est_c3(1) est_c4(1) est_c1(1)], [est_c1(2) est_c2(2) est_c3(2) est_c4(2) est_c1(2)], [est_c1(3) est_c2(3) est_c3(3) est_c4(3) est_c1(3)], '-.', 'LineWidth',4.16,'Color','#EDB120');
       ...
       %% 레퍼런스 부모니터 프레임 시각화
        ...
        xlim([-1000 1000]); ylim([-1000 200]); zlim([-1000 1000]);
        xlabel('\itx','FontSize',20); ylabel('\ity','FontSize',20); zlabel('\itz','FontSize',20);
        view(180,90)
        hold off
