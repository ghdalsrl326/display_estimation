function Cost = Objective_function_mle_2disp(x_first, raw_data_true_first)

azimuth = x_first(1); elevation = x_first(2); r = x_first(3); %camera에서 second_display_postion을 잇는 벡터를 구좌표계로 표현 (camera 좌표계)
sphere_center = [x_first(4); x_first(5); x_first(6)]; % neutral vector == head position(camera 좌표계)
head_coef = x_first(7); % head vector를 eye gaze처럼 활용하기 위한 보정계수
...
target_x = (960 - raw_data_true_first.target_x)*p2mm_x; %processing coordinate -> cam coord system (unit: mm)
target_y = (raw_data_true_first.target_y - 1080)*p2mm_y;%processing coordinate -> cam coord system (unit: mm)
...
head_eul_true = [raw_data_true_first.pose_Rx, raw_data_true_first.pose_Ry, raw_data_true_first.pose_Rz]; % Left-handed positive
