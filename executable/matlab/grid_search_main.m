...
for s = 1:10 % 1번 피험자 ~ 10번 피험자 데이터
    for n = 1:5 % 1번 부모니터 위치 ~ 5번 부모니터 레퍼런스 위치 (우축, 중앙, 좌측, 좌측, 우측)
       ...
       %% Grid Search Optimization
        ...
        title_cand = convertStringsToChars(strcat('..\..\dataset\',path,ss(s),sub_file(n),'_cand_final'));
...
%         mu_head = [mean(error_head_second_x), mean(error_head_second_y)]; 
       ...
       %% 결과 저장
        ...
        title = convertStringsToChars(strcat('..\..\dataset\',path,ss(s),sub_file(n),'_result_final'));
